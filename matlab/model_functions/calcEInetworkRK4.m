...
% For voltage in the previous and current step
% TODO Does it get shifted each timestep?
Vi = zeros(Ni*reg, 2);
...
% Initialize Voltage in 'previous' step for all interneurons
if fixed_initcond == 2
...
RandStream.setGlobalStream(s);
if fixed_initcond == 2
    Ve(:,2) = -20*rand(Ne*reg,1)-50; %uniformly distributed between -50 and -70 mV
elseif fixed_initcond == 1
    Ve(:,2) = -75;
end
...
for T = 2:tnumb
    % Saving indices of previous and current run to use on arrays
